...
(function(){
  'use strict';
  if(window.__IMEX_FASE1_OFICIAL__) return;
  window.__IMEX_FASE1_OFICIAL__ = true;
  ...
  window.imexBenefToggle=function(btn){ const item=btn.closest('.benefit-item'); if(!item) return; item.classList.toggle('active'); const title=item.querySelector('h3')?.textContent||''; document.querySelectorAll(`.progress-chip`).forEach(c=>{ if(title && c.textContent.includes(title)){c.classList.add('done'); c.textContent='✓ '+title;} }); };
  ...
  window.imexAccToggle=function(btn){ const item=btn.closest('.acc-item'); if(item)item.classList.toggle('open'); };
