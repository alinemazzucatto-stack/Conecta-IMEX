...
async function doLogin() {
  ...
  const load  = document.getElementById('lLoading');
  const dbg   = document.getElementById('lDebug');

  // Sempre mostrar debug
  dbg.style.display = 'block';
  dbg.textContent   = '';
  const log2 = (msg) => { dbg.textContent += msg + '\n'; dbg.scrollTop = dbg.scrollHeight; };
  ...
  if (location.protocol === 'file:') {
    showErr('⚠️ Abra via servidor HTTP, não pelo explorador de arquivos.');
    document.getElementById('lFileWarning').style.display = 'block';
    return;
  }
  ...
  try {
    ...
    log2('✅ Firebase OK (' + firebase.apps.length + ' app(s))');
  } catch(e) {
    log2('❌ Firebase não carregou: ' + e.message);
    showErr('❌ Firebase não carregou. Verifique sua conexão e recarregue a página.');
    btn.disabled = false;
  ...
  try {
    ...
    log2('⏳ Buscando colaborador em grh_colabs...');
    let udata;
    try {
      let snap = await db.collection('grh_colabs')
        .where('email', '==', email)
        .limit(1)
        .get();

      // Fallback para bases antigas/unificadas, caso exista variação por unidade.
      if (snap.empty) {
        snap = await db.collection('meta_grh_colabs')
          .where('email', '==', email)
          .limit(1)
          .get();
      }
      if (snap.empty) {
        snap = await db.collection('xpert_grh_colabs')
          .where('email', '==', email)
          .limit(1)
          .get();
      }
      ...
      const perfis = Array.isArray(colab.perfis)
        ? colab.perfis.map(p => String(p).toLowerCase().trim()).filter(Boolean)
        : [String(colab.perfil || 'colaborador').toLowerCase().trim()];

      const perfilEscolhido = String(loginRole || sessionStorage.getItem('imexPreferredRole') || 'colaborador').toLowerCase().trim();
      const podeUsarEscolhido = perfilEscolhido === 'colaborador' || perfis.includes(perfilEscolhido);
      const roleBase = podeUsarEscolhido
        ? perfilEscolhido
        : (perfis.includes('rh') ? 'rh' : (perfis.includes('gestor') ? 'gestor' : 'colaborador'));
      ...
      udata = {
        ...
        role: roleBase,
        perfis,
        unidade: (colab.unidade || 'meta').toString().toLowerCase().includes('xpert') ? 'xpert' : 'meta',
        setor: colab.setor || '',
      ...
      await colabDoc.ref.update({
        authUid: uid,
        ultimoAcesso: new Date().toISOString()
      });
      log2('✅ Colaborador encontrado em ' + colabDoc.ref.parent.path);
      log2('👥 Perfis: ' + perfis.join(', '));
    } catch(firestoreErr) {
      log2('❌ Erro ao buscar colaborador: ' + firestoreErr.message);
      throw firestoreErr;
    }

    log2('👤 Role base: ' + udata.role);
    role = (udata.role || 'colaborador').toLowerCase().trim();
    if (!PLBL[role]) { log2('⚠️ Role inválido, usando colaborador.'); role = 'colaborador'; }
    _roleReal = (udata.perfis || []).includes('rh') ? 'rh' : ((udata.perfis || []).includes('gestor') ? 'gestor' : role);
    try { sessionStorage.setItem('imexPreferredRole', role); } catch(e) {}

    currentUserData = udata;
    currentUnidade  = udata.unidade || 'meta';
    sessionStorage.setItem('userRole',  role);
    sessionStorage.setItem('userPerfis', JSON.stringify(udata.perfis || ['colaborador']));
    sessionStorage.setItem('userName',  udata.nome  || email);
    sessionStorage.setItem('userEmail', udata.email || email);
    sessionStorage.setItem('userDocId', udata.docId || '');
    localStorage.setItem('usuarioLogado', JSON.stringify(udata));

    log2('✅ Entrando como: ' + PLBL[role] + ' · ' + currentUnidade);
    ...
    buildSidebar();
    const pU = document.getElementById('pUnidade');
    if (pU) pU.textContent = currentUnidade === 'xpert' ? '⚡ Xpert' : '🏢 Meta';

    setTimeout(async () => {
      try { await updateHero(); }    catch(e) { console.warn('updateHero:', e.message); }
      try { await renderAll(); }     catch(e) { console.warn('renderAll:', e.message); }
      try { enforcePermissions(); }  catch(e) {}
      if (role === 'colaborador') {
        try { await updateColResumo(); } catch(e) { console.warn('updateColResumo:', e.message); }
      }
      try { renderMeuDesenvolvimento(); } catch(e) { console.warn('renderMeuDesenvolvimento:', e.message); }
    }, 0);

    try { log('Login realizado', 'Perfil: ' + PLBL[role], '🔐'); } catch(e) {}

  } catch(e) {
    log2('❌ ERRO code: ' + (e.code || 'sem código'));
    log2('❌ ERRO msg:  ' + e.message);
    const msgs = {
      ...
      'auth/api-key-not-valid':      '❌ Chave do Firebase inválida. Contate o administrador.',
    };
    showErr(msgs[e.code] || ('❌ ' + (e.message || 'Erro desconhecido. Veja o log abaixo.')));
  } finally {
    btn.disabled       = false;
    load.style.display = 'none';
  }
...
async function clearColabs() {
  ...
  const all = await getCols();
  const batch = db.batch();
  all.forEach(c => batch.delete(db.collection(col('colabs')).doc(c.id)));
  await batch.commit();
  _cacheColabs = null;
  renderColabs(); updateColStats();
